...
transactions_clean = []
for sublist in daily_transactions_split:
  single_clean_transaction= []
  for item in sublist:
    single_clean_transaction.append(item.replace("\n", "").strip(" "))
  transactions_clean.append(single_clean_transaction)
...
for item in thread_sold:
  if item.find("&") == -1:
    ...
    thread_list = []
    threads_split = item.split("&")
    for thread in threads_split:
